...
--this table is shared between child and parent objects acting as protected fields
local tSharedFields = {};
...
local function class_ctor(class, ...)
	...
	if (type(class.__construct) == "function") then
		...
		--local args = arg or select(...);
		--check the first argument to see

		tSharedFields[instance] = setmetatable({}, {__type = "_shared_fields_"});
		class.__construct(instance, tSharedFields[instance], ...);
	end
		...
		__call  = function(class_object, members)	-- the actual definer, this puts the class in _G and adds the methods (no longer puts it in _G...has been localized)
			...
			class_object.super = function(instance, ...)
				...
				if (parent and type(parent.__construct) == "function") then					
					...
					parent.__construct(setmetatable(instance, getmetatable(parent).__instance_mt), tSharedFields[instance], ...);
