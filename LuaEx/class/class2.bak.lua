...
kit = {
    ...
    build = function(sName, tMetamethods, tStaticPublic, tPrivate, tProtected, tPublic, cExtendor, vImplements, bIsFinal)
		--check the input
        ...
        local cParent = tKit.parent;

		if (cParent) then
			local tKitRepo 	 = tKit.repo.byobject;
			--local tChildren = tKitRepo[cParent];
            local tParentKit = tKitRepo[cParent];

			--tChildren.byname[sName] = tKit;
            tParentKit.children.byname[sName] = tKit;
		end
...
local function class(	_IGNORE_, sName,
    ...
    kit.build(sName, tMetamethods, tStaticPublic, tPrivate, tProtected, tPublic, cExtendor, vImplements, bIsFinal);
