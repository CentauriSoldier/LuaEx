...
local type 			= type;
--PLANNED inheritence and struct interfaces

local function dummy() end --INCOMPLETE TODO change this to use an actrual table for newindex, and index

local function errbit()--TODO change these messages to reflect the new type names
...
function kit.build(sSubType, tProperties)
    assert(rawtype(sSubType) == "string" and sSubType:gsub("[%s]", "") ~= "", "Error creating struct factory. Argument 1 (Subtype) must be a non-blank string.");--TODO should I check for variable compliance? Or, do I even need a name? Well, maybe for type.
...
local function createfactory(__IGNORE__, sSubType, tProperties)--, bPrivate)
	--[[
	███████╗░█████╗░░█████╗░████████╗░█████╗░██████╗░██╗░░░██╗
	██╔════╝██╔══██╗██╔══██╗╚══██╔══╝██╔══██╗██╔══██╗╚██╗░██╔╝
	█████╗░░███████║██║░░╚═╝░░░██║░░░██║░░██║██████╔╝░╚████╔╝░
	...
	for vKey, vValue in pairs(tProperties) do --QUESTION should I enforce string keys?
		nKeyCount = nKeyCount + 1;
		tConstraint[vKey] = {
			type 			= type(vValue),
			defaultvalue 	= vValue,
